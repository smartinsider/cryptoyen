...
unsigned CMasternode::Level(const CTxIn& vin, int blockHeight)
{
    ...
    LogPrintf("CMasternode::Level() = Level Info = %s\n",vin_val);
...
bool CMasternode::IsDepositCoins(const CTxIn& vin, CAmount& vin_val)
{
    ...
    //LogPrintf("CMasternode::IsDepositCoins() = Info 1 = %s\n",vin_amount);

    if(!IsDepositCoins(vin_amount))
        return false;

    //LogPrintf("CMasternode::IsDepositCoins() = Info 2 = %s\n",vin_amount);
