...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "cryptoyen_modern" is a composite category enabling all YEN-related debug output
            if (ptrCategory->count(string("cryptoyen_modern"))) {
...
#else
    const char* pszModule = "cryptoyen_modern";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\cryptoyen-anime
// Windows >= Vista: C:\Users\Username\AppData\Roaming\cryptoyen-anime
// Mac: ~/Library/Application Support/cryptoyen-anime
// Unix: ~/.cryptoyen-anime
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "cryptoyen-anime";
#else
    fs::path pathRet;
    ...
    return pathRet / "cryptoyen-anime";
#else
    // Unix
    return pathRet / ".cryptoyen-anime";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cryptoyen_modern.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty cryptoyen_modern.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override cryptoyen_modern.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cryptoyen_modernd.pid"));
