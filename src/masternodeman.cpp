...
bool CMasternodeMan::Add(CMasternode& mn)
{
    ...
    LogPrint("masternode", "CMastersnodeMan: Adding new Masternode %s - %i now\n", mn.vin.prevout.hash.ToString(), size() + 1);
...
CMasternode* CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, unsigned mnlevel, bool fFilterSigTime, unsigned& nCount)
{
    ...
    */
    ...
    if (fFilterSigTime && nCount < nMnCount / 2) return GetNextMasternodeInQueueForPayment(nBlockHeight, mnlevel, false, nCount);
...
int CMasternodeMan::GetMasternodeRank(const CTxIn& vin, int64_t nBlockHeight, int minProtocol, bool fOnlyActive)
{
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
        ...
        //SPORK_4_MASTERNODE_PAYMENT_ENFORCEMENT ACTIVATE
        nMasternode_Age = GetAdjustedTime() - mn.sigTime;
        if ((nMasternode_Age) < nMasternode_Min_Age) {
...
std::vector<pair<int, CMasternode> > CMasternodeMan::GetMasternodeRanks(int64_t nBlockHeight, int minProtocol)
{
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
		...
		//MASTERNODE PORT PROTECTION DEACTIVATE
        //if (mn.protocolVersion < minProtocol) continue;
        if (!mn.IsEnabled()) {
            vecMasternodeScores.push_back(make_pair(17171, mn));
            continue;
        }
