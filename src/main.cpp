...
/** Fees smaller than this (in ucryptoyen_modern) are considered zero fee (for relaying and mining)
...
{
    int64_t ret = 0;
    blockValue = GetBlockValue(nHeight);
    switch(mnlevel)
    {
        case 1:
        ...
        case 2:
            ...
            break;
        case 3:
            ret = blockValue * 30 / 100; //30% of the block reward | 4000 YEN
            break;
        case 4:
            ret = blockValue * 40 / 100; //40% of the block reward | 25000 YEN
            break;
...
{
    RenameThread("cryptoyen_modern-scriptch");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if ((Params().NetworkIDString() == "regtest") && block.nBits != GetNextWorkRequired(pindexPrev))
