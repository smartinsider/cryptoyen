...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    // TOFIXED >> nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval
    while (nStakeModifierTime < pindexFrom->GetBlockTime()) {
        if (!pindexNext) {
            // Should never happen
...
bool Stake(CStakeInput* stakeInput, unsigned int nBits, unsigned int nTimeBlockFrom, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    ...
    // TOFIXED >> NEED FOR GOOD STAKING
    if (!stakeInput->GetModifier(nStakeModifier))
        return error("failed to get kernel stake modifier");// TOFIXED >> error("failed to get kernel stake modifier");
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    CCryptoYenStake* cryptoyen_modernInput = new CCryptoYenStake();
    cryptoyen_modernInput->SetInput(txPrev, txin.prevout.n);
    stake = std::unique_ptr<CStakeInput>(cryptoyen_modernInput);
