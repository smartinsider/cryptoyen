...
/**
 * Fees smaller than this (in ucryptoyen_modern) are considered zero fee (for transaction creation)
...
CAmount CWalletTx::GetUnlockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && vout[i].nValue == 800 * COIN) continue; // do not count MN-like outputs
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        else if (fMasterNode && vout[i].nValue == 800 * COIN) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_NOT10000IFMN) {
					found = !(fMasterNode && (pcoin->vout[i].nValue == 800 * COIN || pcoin->vout[i].nValue == 4000 * COIN || pcoin->vout[i].nValue == 20000 * COIN || pcoin->vout[i].nValue == 100000 * COIN));

                } else if (nCoinType == ONLY_10000) {
					if (pcoin->vout[i].nValue == 800 * COIN || pcoin->vout[i].nValue == 4000 * COIN || pcoin->vout[i].nValue == 20000 * COIN || pcoin->vout[i].nValue == 100000 * COIN){
			        	found = true;
					}
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (fMasterNode && out.tx->vout[out.i].nValue == 800 * COIN) continue; //masternode input
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal MN collateral CryptoYen.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal MN collateral CryptoYen.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-cryptoyen_modern-address
                    CScript scriptChange;
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (stakeInput->GetValue() < 5 * COIN ) {
            //LogPrintf("CreateCoinStake() : min stake value need to be more then 5 coins \n");
            continue;
        }
        ...
        if (Stake(stakeInput.get(), nBits, block.GetBlockTime(), nTxNewTime, hashProofOfStake)) {
            ...
            if (txNew.vout.size() == 3) {
		        ...
		        //txNew.vout.push_back(CTxOut(nCredit - nMinFee - txNew.vout[1].nValue, txNew.vout[1].scriptPubKey));
